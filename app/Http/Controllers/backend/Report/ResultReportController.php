...
class ResultReportController extends Controller
{
    ...
    if ($singleResult == true) {
		...
		$data['st_name'] = AssignStudent::where('year_id',$year_id)->where('class_id',$class_id)->get();
		$data['marks'] = StudentMarks::where('year_id',$year_id)->where('class_id',$class_id)->where('exam_type_id',$exam_type_id)->get();
		// $data['st_name1'] = User::where('id',$st_name->student_id)->get();
