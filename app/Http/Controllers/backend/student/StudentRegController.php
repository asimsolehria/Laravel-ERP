...
class StudentRegController extends Controller
{
 ...
 public function StudentUpdatePromotion(Request $request,$student_id){
	...
	$attendance = StudentAttendance::where('student_id',$request->id)->first();
	$user = User::where('id',$student_id)->first();
	$assign = AssignStudent::where('student_id',$student_id)->first();
	$discount = DiscountStudent::where('assign_student_id',$student_id)->first();

	dd($attendance->toArray());

		if ($user)
		{
			User::where('id',$student_id)->first()->delete();
		}

		if ($assign)
		{
			AssignStudent::where('student_id',$student_id)->first()->delete();
		}

		if ($discount)
		{
			DiscountStudent::where('assign_student_id',$student_id)->first()->delete();
		}

		if ($attendance)
		{
			StudentAttendance::where('student_id',$request->id)->first()->delete();
		}
